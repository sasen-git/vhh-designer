...
try:
    ...
    HAS_ANTPACK = False
    print("Note: AntPack not found. Using regex-based extraction.")
    print("      For accurate IMGT numbering: pip install antpack")

# ============================================================
# IMGT POSITION MAPPINGS
# ============================================================

# Mapping from old FR-based names to IMGT numbers (for regex extraction)
# This assumes standard VHH FR2 starting with W at IMGT 41
FR_TO_IMGT = {
    # FR2 positions
    'FR2_1': 41, 'FR2_2': 42, 'FR2_3': 43, 'FR2_4': 44, 'FR2_5': 45,
    'FR2_6': 46, 'FR2_7': 47, 'FR2_8': 48, 'FR2_9': 49, 'FR2_10': 50,
    'FR2_11': 51, 'FR2_12': 52, 'FR2_13': 53, 'FR2_14': 54,
    # FR3 positions (approximate - FR3 starts ~IMGT 66)
    'FR3_1': 66, 'FR3_2': 67, 'FR3_3': 68, 'FR3_4': 69, 'FR3_5': 70,
    'FR3_6': 71, 'FR3_7': 72, 'FR3_8': 73, 'FR3_9': 74, 'FR3_10': 75,
    'FR3_11': 76, 'FR3_12': 77, 'FR3_13': 78, 'FR3_14': 79, 'FR3_15': 80,
    'FR3_16': 81, 'FR3_17': 82, 'FR3_18': 82, 'FR3_19': 83, 'FR3_20': 84,
    'FR3_21': 85, 'FR3_22': 86, 'FR3_23': 87, 'FR3_24': 88, 'FR3_25': 89,
    'FR3_26': 90, 'FR3_27': 91, 'FR3_28': 92, 'FR3_29': 93, 'FR3_30': 94,
    # FR1 positions (FR1 starts at IMGT 1)
    'FR1_1': 1, 'FR1_5': 5, 'FR1_10': 10, 'FR1_20': 20, 'FR1_25': 25,
    # FR4 positions (FR4 starts ~IMGT 103)
    'FR4_1': 103, 'FR4_2': 104, 'FR4_3': 105, 'FR4_4': 106, 'FR4_5': 107,
    'FR4_6': 108, 'FR4_7': 109, 'FR4_8': 110, 'FR4_9': 111, 'FR4_10': 112, 'FR4_11': 113,
}

# Reverse mapping
IMGT_TO_FR = {v: k for k, v in FR_TO_IMGT.items()}

# VHH Hallmark positions (Vincke et al.)
HALLMARK_IMGT = {
    42: 'hallmark_42',  # F or Y (aromatic) - VL interface
    49: 'hallmark_49',  # E (in VHH) vs G (in VH)
    50: 'hallmark_50',  # R (in VHH) vs L (in VH)  
    52: 'hallmark_52',  # G, L, F, or A (variable)
}

# Key Vernier zone positions in IMGT numbering
VERNIER_POSITIONS_IMGT = [42, 47, 49, 50, 52, 53, 71, 73, 77, 81]

# Position names to use in output (IMGT-based)
# Old Vernier: FR2_4, FR2_11, FR2_12, FR2_14, FR3_6, FR3_8, FR3_12, FR3_16
# New: IMGT44, IMGT51, IMGT52, IMGT54, IMGT71, IMGT73, IMGT77, IMGT81
VERNIER_PATTERN_POSITIONS = ['IMGT44', 'IMGT51', 'IMGT52', 'IMGT54', 'IMGT71', 'IMGT73', 'IMGT77', 'IMGT81']
...
# Global IMGT annotator (lazy initialization)
_imgt_annotator = None

def get_imgt_annotator():
    """Get or create the IMGT annotator (singleton)."""
    global _imgt_annotator
    if _imgt_annotator is None and HAS_ANTPACK:
        _imgt_annotator = SingleChainAnnotator(chains=['H'], scheme='imgt')
    return _imgt_annotator


def extract_frameworks_imgt(full_seq: str, cdr1: str, cdr2: str, cdr3: str) -> Dict:
    """
    Extract frameworks using AntPack for proper IMGT numbering.
    
    Returns positions named as IMGT## (e.g., IMGT42, IMGT49).
    Falls back to regex-based extraction if AntPack unavailable.
    """
    if not HAS_ANTPACK:
        # Fall back to regex and convert names
        result = extract_frameworks(full_seq, cdr1, cdr2, cdr3)
        if result['valid']:
            # Convert FR-based names to IMGT names
            imgt_positions = {}
            for fr_name, aa in result['positions'].items():
                if fr_name in FR_TO_IMGT:
                    imgt_num = FR_TO_IMGT[fr_name]
                    imgt_positions[f'IMGT{imgt_num}'] = aa
                elif fr_name.startswith('CDR'):
                    imgt_positions[fr_name] = aa  # Keep CDR positions as-is
            result['positions'] = imgt_positions
        return result
    
    seq = str(full_seq).upper().replace('-', '').replace('.', '').replace(' ', '')
    ...
    try:
        annotator = get_imgt_annotator()
        numbering, pid, chain, err = annotator.analyze_seq(seq)
        
        if numbering is None or len(numbering) == 0:
            return result
        
        trimmed_seq, trimmed_nums, _, _ = annotator.trim_alignment(
            seq, (numbering, pid, chain, err)
        )
        ...
        # Build regions and IMGT positions
        regions = {'FR1': '', 'CDR1': '', 'FR2': '', 'CDR2': '', 'FR3': '', 'CDR3': '', 'FR4': ''}
        positions = {}
        
        for aa, label, imgt_pos in zip(trimmed_seq, labels, trimmed_nums):
            # Get IMGT number
            ...
            # Store position with IMGT name
            if insertion:
                pos_key = f'IMGT{imgt_num}{insertion}'
            else:
                pos_key = f'IMGT{imgt_num}'
            positions[pos_key] = aa
            
            # Also store numeric key for easier lookup
            positions[imgt_num] = aa
            
            # Build region sequences
            normalized = label_map.get(label.lower(), None)
            if normalized:
                regions[normalized] += aa
        
        # Add CDR anchor positions
        cdr1_str = str(cdr1) if cdr1 else ''
        cdr2_str = str(cdr2) if cdr2 else ''
        cdr3_str = str(cdr3) if cdr3 else ''
        
        if len(cdr1_str) >= 2:
            positions['CDR1_first'] = cdr1_str[0]
            positions['CDR1_last'] = cdr1_str[-1]
            positions['CDR1_2nd'] = cdr1_str[1] if len(cdr1_str) > 1 else '-'
            positions['CDR1_2ndlast'] = cdr1_str[-2] if len(cdr1_str) > 1 else '-'
        
        if len(cdr2_str) >= 2:
            positions['CDR2_first'] = cdr2_str[0]
            positions['CDR2_last'] = cdr2_str[-1]
            positions['CDR2_2nd'] = cdr2_str[1] if len(cdr2_str) > 1 else '-'
            positions['CDR2_2ndlast'] = cdr2_str[-2] if len(cdr2_str) > 1 else '-'
        
        if len(cdr3_str) >= 2:
            positions['CDR3_first'] = cdr3_str[0]
            positions['CDR3_last'] = cdr3_str[-1]
            positions['CDR3_2nd'] = cdr3_str[1] if len(cdr3_str) > 1 else '-'
            positions['CDR3_2ndlast'] = cdr3_str[-2] if len(cdr3_str) > 1 else '-'
            positions['CDR3_3rd'] = cdr3_str[2] if len(cdr3_str) > 2 else '-'
            positions['CDR3_3rdlast'] = cdr3_str[-3] if len(cdr3_str) > 2 else '-'
        
        result['fr1'] = regions['FR1']
        result['fr2'] = regions['FR2']
        result['fr3'] = regions['FR3']
        result['fr4'] = regions['FR4']
        result['positions'] = positions
        result['valid'] = bool(regions['FR2'] and regions['FR3'])
        
        return result
        
    except Exception as e:
        # Fall back to regex on error
        return extract_frameworks(full_seq, cdr1, cdr2, cdr3)


def classify_family_imgt(positions: Dict[str, str], cdr3: str, full_seq: str = '') -> str:
    """
    Classify VHH family using IMGT-numbered positions.
    
    Works with positions named as IMGT## or numeric keys.
    """
    # Get hallmark positions (try both string and numeric keys)
    pos42 = positions.get('IMGT42', positions.get(42, '-'))
    pos49 = positions.get('IMGT49', positions.get(49, '-'))
    pos50 = positions.get('IMGT50', positions.get(50, '-'))
    pos52 = positions.get('IMGT52', positions.get(52, '-'))
    
    # Count total cysteines
    if full_seq:
...
class VernierClusterer:
    ...
    Supports both old (FR2_4) and new (IMGT44) position naming.
    """
    
    # Position mappings for Vernier pattern
    # Old: FR2_4, FR2_11, FR2_12, FR2_14, FR3_6, FR3_8, FR3_12, FR3_16
    # IMGT: 44, 51, 52, 54, 71, 73, 77, 81
    VERNIER_OLD = ['FR2_4', 'FR2_11', 'FR2_12', 'FR2_14', 'FR3_6', 'FR3_8', 'FR3_12', 'FR3_16']
    VERNIER_IMGT = [44, 51, 52, 54, 71, 73, 77, 81]
    VERNIER_IMGT_NAMES = ['IMGT44', 'IMGT51', 'IMGT52', 'IMGT54', 'IMGT71', 'IMGT73', 'IMGT77', 'IMGT81']
    
    def __init__(self, use_imgt_names: bool = False):
        self.use_imgt_names = use_imgt_names
    ...
    def _get_position(self, positions: Dict[str, str], old_name: str, imgt_num: int) -> str:
        """Get position value, trying IMGT name first, then old name."""
        # Try IMGT string key first
        val = positions.get(f'IMGT{imgt_num}', '-')
        if val != '-':
            return val
        # Try numeric key
        val = positions.get(imgt_num, '-')
        if val != '-':
            return val
        # Fall back to old name
        return positions.get(old_name, '-')
    
    def add(self, positions: Dict[str, str], 
            cdr1_feat: CDRFeatures, cdr2_feat: CDRFeatures, cdr3_feat: CDRFeatures,
            family: str):
        
        # Build pattern using both naming conventions
        pattern = tuple(
            self._get_position(positions, old, imgt)
            for old, imgt in zip(self.VERNIER_OLD, self.VERNIER_IMGT)
        )
    ...
    def analyze(self, min_size: int = 500) -> Dict:
        ...
        # Use IMGT names in output if requested
        pos_names = self.VERNIER_IMGT_NAMES if self.use_imgt_names else self.VERNIER_OLD
        ...
        for pattern, c in self.clusters.items():
            ...
            results[pattern] = {
                'n': c['n'],
                'pattern': dict(zip(pos_names, pattern)),
...
class HigherOrderRuleFinder:
    ...
    # Position pair mappings
    # Old pairs: (FR2_4, FR3_8), (FR2_4, FR3_12), (FR2_12, FR3_6), (FR2_14, FR3_16), (FR3_6, FR3_12)
    # IMGT pairs: (44, 73), (44, 77), (52, 71), (54, 81), (71, 77)
    POSITION_PAIRS_OLD = [
        ('FR2_4', 'FR3_8'),
        ('FR2_4', 'FR3_12'),
        ('FR2_12', 'FR3_6'),
    ...
    def _pos_name(self, old_name: str, imgt_num: int) -> str:
        """Get position name based on output format preference."""
        return f'IMGT{imgt_num}' if self.use_imgt_names else old_name
    ...
    def add(self, family: str, positions: Dict[str, str],
        ...
        # Count patterns using both naming conventions
        for (old1, old2), (imgt1, imgt2) in zip(self.POSITION_PAIRS_OLD, self.POSITION_PAIRS_IMGT):
            res1 = self._get_position(positions, old1, imgt1)
            res2 = self._get_position(positions, old2, imgt2)
            
            # Get position names for output
            pos1_name = self._pos_name(old1, imgt1)
            pos2_name = self._pos_name(old2, imgt2)
            
            if res1 != '-' and res2 != '-':
                for cdr_feat_name, cdr_val in cdr_features.items():
                    key = (cdr_feat_name, cdr_val, pos1_name, res1, pos2_name)
...
def process_npz(npz_path: str, analyzers: Dict) -> int:
    """Process an NPZ file and add sequences to all analyzers.
    
    Uses IMGT extraction if analyzers['_config']['use_imgt'] is True.
    """
    # Get config
    config = analyzers.get('_config', {})
    use_imgt = config.get('use_imgt', False)
    ...
    for i in iterator:
        try:
            ...
            # Use IMGT or regex extraction based on config
            if use_imgt:
                fw = extract_frameworks_imgt(seq, cdr1, cdr2, cdr3)
            else:
            ...
            if use_imgt:
                family = classify_family_imgt(positions, cdr3, seq)
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='VHH Full Epistasis Analysis - OVERNIGHT v3 (IMGT)')
    ...
    # IMGT mode
    parser.add_argument('--use-imgt', action='store_true',
                        help='Use AntPack for proper IMGT numbering (slower but more accurate)')
    parser.add_argument('--imgt-output-names', action='store_true',
                        help='Use IMGT position names in output (e.g., IMGT42 instead of FR2_2)')
    ...
    # Check IMGT mode requirements
    if args.use_imgt and not HAS_ANTPACK:
        print("WARNING: --use-imgt requested but AntPack not available")
        print("         Install with: pip install antpack")
        print("         Falling back to regex-based extraction")
        args.use_imgt = False
    
    # If using IMGT output names, also use IMGT extraction
    if args.imgt_output_names:
        args.use_imgt = True
    
    start_time = time.time()
    
    print("="*70)
    print("VHH FULL EPISTASIS ANALYSIS - OVERNIGHT v3 (IMGT NUMBERING)")
    print("Memory-optimized with streaming stats")
    print("="*70)
    
    if args.use_imgt:
        print("IMGT MODE: Using AntPack for accurate position numbering")
        if args.imgt_output_names:
            print("OUTPUT: Using IMGT position names (e.g., IMGT42, IMGT49)")
    else:
        print("REGEX MODE: Using regex-based extraction (faster)")
        print("POSITION MAPPING (assumes standard FR2 start):")
    ...
    print(f"Parameters:")
    print(f"  use_imgt: {args.use_imgt}")
    print(f"  imgt_output_names: {args.imgt_output_names}")
    ...
    # Position names for MI - use IMGT names if requested
    if args.imgt_output_names:
        position_names = [
            'IMGT41', 'IMGT42', 'IMGT43', 'IMGT44', 'IMGT45', 'IMGT46', 'IMGT47',
            'IMGT48', 'IMGT49', 'IMGT50', 'IMGT51', 'IMGT52', 'IMGT53', 'IMGT54',
            'IMGT66', 'IMGT67', 'IMGT69', 'IMGT71', 'IMGT73', 'IMGT75', 'IMGT77',
            'IMGT79', 'IMGT81', 'IMGT83', 'IMGT85', 'IMGT87', 'IMGT89',
            'IMGT103', 'IMGT104', 'IMGT105', 'IMGT106', 'IMGT107',
    ...
    analyzers = {
        ...
        'cluster': VernierClusterer(use_imgt_names=args.imgt_output_names),
        'cond': ConditionalModeler(max_samples_per_position=100000),
        'rules': HigherOrderRuleFinder(use_imgt_names=args.imgt_output_names),
        'mi': MutualInformationAnalyzer(position_names, max_sequences=args.mi_max_seqs),
    }
    
    # Store config for later
    analyzers['_config'] = {
        'use_imgt': args.use_imgt,
        'imgt_output_names': args.imgt_output_names,
    }
