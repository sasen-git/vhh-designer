...
def extract_cdr3_from_sequence(sequence: str) -> str:
    ...
    Handles variants:
    - FR2 anchors: WFRQ, WYRQ, WVRQ, WFRR, WYRK, etc.
    - CDR3 endings: WGQG, WSQG, WVQG, WG, WS, and non-W endings like SSGQ, YRGQ
    """
    seq = sequence.upper().replace('-', '').replace('.', '')
    
    # Step 1: Find FR2 anchor - accept various patterns
    fr2_match = re.search(r'W[YFVILA]R[QRK]', seq[10:60])  # Start at 10 for truncated sequences
    if not fr2_match:
        fr2_match = re.search(r'W[YFVILA][RK][QRK]', seq[10:60])
    if not fr2_match:
        fr2_match = re.search(r'W[A-Z]R[A-Z]', seq[10:60])
    
    if fr2_match:
        fr2_start = 10 + fr2_match.start()  # Match the search range [10:60]
        search_start = fr2_start + 50  # Skip FR2 (14) + CDR2 (~17) + part of FR3 (~20)
    else:
        search_start = 80  # Fallback
    
    # Step 2: Find FR4 start (CDR3 end)
    # FR4 patterns: WGQGTLVTVSS, WGQGTQVTVSS, WVQG, SSGQ, etc.
    # The key is to find QGTLVTVSS or QGTQVTVSS near the end
    
    cdr3_end = None
    
    # Try to find FR4 signature near end
    fr4_patterns = [
        r"W[GSVA]QG[TL][LQ]VTVSS",  # Standard WGQGTLVTVSS
        r"W[GSVAR]QG[TL]",          # WGQGT, WRQGT
        r"W[GSR][QRGA]G[TL]",       # WGR, WSA variants
        r"[A-Z]{2}QG[TL][LQ]VTVSS", # Non-W start
        r"W[GSVA]QG",               # Just WGQG or WVQG
        r"W[GSR][QRGA]G",           # WGR short
        r"[A-Z]{2}QG[TL]",          # Non-W start XXQGT
    ]
    for pattern in fr4_patterns:
        match = re.search(pattern, seq[search_start:])
        if match:
            cdr3_end = search_start + match.start()
            break
    
    # Fallback: look for QGTLVTVSS or QGTQVTVSS near end
    if cdr3_end is None:
        qgt_match = re.search(r'QG[TL][LQ]VTVSS', seq[-25:])
        if qgt_match:
            # Find what's 2 positions before QG
            qg_pos = len(seq) - 25 + qgt_match.start()
            cdr3_end = qg_pos - 2 if qg_pos >= 2 else None
    
    # Last fallback: look for WG or WS anywhere after search_start
    if cdr3_end is None:
        for end_pos in range(len(seq) - 15, search_start, -1):
            if seq[end_pos:end_pos+2] in ['WG', 'WS', 'WV']:
                cdr3_end = end_pos
                break
    ...
    # Step 3: Find CDR3 start (C before cdr3_end)
    cdr3_start = None
    for c_pos in range(cdr3_end - 5, max(search_start - 10, cdr3_end - 40), -1):
        if c_pos >= 0 and seq[c_pos] == 'C':
            cdr3_start = c_pos
            break
    ...
    # IMGT CDR3: starts AFTER the conserved Cys (pos 104), at position 105
    # So we return from cdr3_start + 1 (skip the C)
...
def extract_frameworks_from_sequence(sequence: str) -> Optional[FrameworkProfile]:
    ...
    if len(seq) < 80:  # Lowered to handle truncated sequences
        return None
    
    # Find FR2 anchor - accept various patterns
    fr2_match = re.search(r'W[YFVILA]R[QRK]', seq[10:60])  # Start at 10 for truncated sequences
    if not fr2_match:
        fr2_match = re.search(r'W[YFVILA][RK][QRK]', seq[10:60])
    if not fr2_match:
    ...
    # Find CDR3 end by looking for FR4 start
    search_start = fr2_end + 40
    
    cdr3_end = None
    
    # Try to find FR4 signature near end
    fr4_patterns = [
        r'W[GSVAR]QG[TL][LQ]VTVSS', # Standard WGQGTLVTVSS
        r'W[GSVAR]QG[TL]',          # WGQGT, WRQGT
        r'W[GSR][QRGA]G[TL]',       # WGR, WSA variants
        r'[A-Z]{2}QG[TL][LQ]VTVSS', # Non-W start like SSGQGTLVTVSS
        r'W[GSVA]QG',               # Just WGQG or WVQG
        r'W[GSR][QRGA]G',           # WGR short
        r'[A-Z]{2}QG[TL]',          # Non-W start XXQGT
    ]
    ...
    # IMGT: FR3 INCLUDES the conserved Cys at position 104
    cdr2_start = fr2_end
    cdr2_approx_len = 17
    fr3_start = cdr2_start + cdr2_approx_len
    fr3_end = cdr3_start + 1  # Include the C in FR3
...
def analyze_batch(input_file: str, 
    ...
    for idx, row in df.iterrows():
        ...
        sequence = ''  # Will be set if in full sequence mode
        ...
        try:
            ...
            # Store result with original sequence
    ...
    output_data = []
    for r, original_seq in results:
        row = {
            'id': r.id,
            'original_sequence': original_seq,  # Add original sequence right after ID
