...
def anarci_imgt_numbering(seqs: List[str]) -> List[Optional[Dict[int, str]]]:
    """
    Run ANARCI on a list of VH/VHH sequences.
    Returns list of dicts mapping IMGT position -> residue, or None if invalid.
    """
    anarci = get_anarci()
    # ANARCI expects list of tuples (name, seq)
    inp = [(f"s{i}", s) for i, s in enumerate(seqs)]
    # scheme='imgt' provides IMGT numbering; allow H only
    results = anarci(inp, scheme="imgt", allow={"H"}, output=False)
    numbered = results[0]  # list aligned to inp
    out: List[Optional[Dict[int, str]]] = []
    for item in numbered:
        if item is None:
            out.append(None)
            continue
        # item is list per chain; we requested allow H, so take first present
        # format: [ (chain_type, numbering, start, end) , ...]
        chain_entries = item
        if not chain_entries:
            out.append(None)
            continue
        # choose heavy chain entry
        entry = None
        for ce in chain_entries:
            if ce and ce[0] == "H":
                ...
                break
        if entry is None:
            entry = chain_entries[0]
        # entry[1] is numbering: list of ((pos, ins), aa) where pos is int
        numbering = entry[1]
        d: Dict[int, str] = {}
        for (pos, ins), aa in numbering:
            if aa == "-":
                continue
            # Ignore insertion code for now: collapse into base position if any
            # If there are insertions, last one wins; this is fine for our per-position "fixed" columns.
            try:
                p = int(pos)
            except Exception:
                continue
    ...
    for cstart, cend in iter_indices(n, chunk_size):
        ...
        if use_anarci:
            ...
            for bstart, bend in iter_indices(len(chunk_seqs), anarci_batch_size):
                imgt_dicts.extend(anarci_imgt_numbering(chunk_seqs[bstart:bend]))
                done_batches += 1
                if show_progress:
                    _progress_update()
        else:
