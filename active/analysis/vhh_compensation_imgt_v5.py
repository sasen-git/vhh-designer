...
class StreamingCompensationAnalyzer:
    ...
    """
    def __init__(self, target_positions: Iterable[int], strict_fixed_lengths: bool = False):
        self.target_positions = set(int(p) for p in target_positions)
        self.strict_fixed_lengths = strict_fixed_lengths
    ...
    def update(self, fr1: str, cdr1: str, fr2: str, cdr2: str, fr3: str, cdr3: str, fr4: str,
        ...
        """Stream a single annotated sequence into the counters.

        Max-coverage behavior (default): we count a sequence for a given IMGT position
        iff that position is present in the provided FR substring (bounds check per-position).
        If strict_fixed_lengths=True, we require fixed-length FR strings for any region needed
        (legacy behavior, lower coverage).
        """
        self.total_sequences_seen += 1

        # We need CDR3 to build conditions; skip rows without it.
        if not cdr3:
            return

        # Optional legacy strictness (kept for debugging)
        if self.strict_fixed_lengths:
        ...
        # Count the row as "used" if it contributed to at least one target position.
        if contributed > 0:
...
def process_csv_streaming(csv_path: str, output_dir: str, chunk_size: int,
                          ...
                          strict_fixed_lengths: bool = False, drop_definitional: bool = True):
    os.makedirs(output_dir, exist_ok=True)
    target_positions = choose_positions(positions_mode)
    analyzer = StreamingCompensationAnalyzer(target_positions, strict_fixed_lengths=strict_fixed_lengths)
...
def main():
    ...
    ap.add_argument("--strict-fixed-lengths", action="store_true",
                    help="Legacy: require fixed-length FR strings for target regions (lower coverage).")
    ap.add_argument("--keep-definitional", action="store_true",
                    help="Keep definitional hallmark rules (e.g., hallmarks=FERG -> IMGT42=F). Default filters these out.")
    args = ap.parse_args()
    ...
    process_csv_streaming(
        ...
        strict_fixed_lengths=args.strict_fixed_lengths,
        drop_definitional=(not args.keep_definitional),
    )
