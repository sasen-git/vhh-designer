...
class StreamingCompensationAnalyzer:
    ...
    def update(self, fr1: str, cdr1: str, fr2: str, cdr2: str, fr3: str, cdr3: str, fr4: str,
        ...
        # Sanity check fixed-length regions; skip rows that would shift IMGT offsets.
        if fr1 and len(fr1) != EXPECTED_FIXED_LENGTHS["fr1"]:
            return
        if cdr1 and len(cdr1) != EXPECTED_FIXED_LENGTHS["cdr1"]:
            return
        if fr2 and len(fr2) != EXPECTED_FIXED_LENGTHS["fr2"]:
            return
        if cdr2 and len(cdr2) != EXPECTED_FIXED_LENGTHS["cdr2"]:
            return
        if fr3 and len(fr3) != EXPECTED_FIXED_LENGTHS["fr3"]:
            return
        if fr4 and len(fr4) != EXPECTED_FIXED_LENGTHS["fr4"]:
            return
