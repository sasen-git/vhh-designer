...
def annotate_imgt_and_markers(ax, msa: MultipleSeqAlignment, lead_rows, wrap_len: int,
    ...
    # Which y is visually lower (bigger pixel y)?
    p0 = ax.transData.transform((0, y0))[1]
    p1 = ax.transData.transform((0, y1))[1]
    y_bottom = y0 if p0 > p1 else y1   # visual bottom in data coords

    # Now decide whether "down" in data is + or -
    # If increasing y goes down on screen, then down_sign = +1, else -1
    p_up = ax.transData.transform((0, y_bottom + 1))[1]
    down_sign = +1 if p_up > ax.transData.transform((0, y_bottom))[1] else -1

    # Place rows below the alignment
    y_nums = y_bottom + down_sign * 1.15
    y_mark = y_bottom + down_sign * 1.65
