...
def anarci_imgt_numbering(seqs: List[str], quiet: bool = True) -> List[Optional[Dict[int, str]]]:
    ...
    # ANARCI can be chatty (hmmer fallback messages). Optionally silence stderr.
    import io
    import contextlib

    try:
        ctx = contextlib.redirect_stderr(io.StringIO()) if quiet else contextlib.nullcontext()
        with ctx:
            try:
                numbered, alignment_details, hit_tables = anarci(inp, scheme="imgt", allow=set(["H"]))
            except TypeError:
                # Some versions don't accept allow=; fall back.
                numbered, alignment_details, hit_tables = anarci(inp, scheme="imgt")
    except Exception:
