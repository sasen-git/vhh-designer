...
class VernierClusterAnalyzer:
    ...
    def add(self, family: str, imgt: Dict[int, str], cdr3_seq: str):
        ...
        self._clusters[key]["cdr3_length"].update(float(f["length"]))
        self._clusters[key]["cdr3_charge"].update(float(f["charge"]))
        self._clusters[key]["cdr3_aromatic"].update(float(arom))
...
"""
    ...
    vernier_cluster_analyzer = VernierClusterAnalyzer(min_positions=3)
    ...
    t0 = time.time()
    with open(os.path.join(args.output, "analysis_vernier_clusters_v7.json"), "w") as f:
