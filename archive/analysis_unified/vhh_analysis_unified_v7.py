...
def assert_rules_have_suggestions(rules, where="all_rules", show=5):
    bad = []
    for i, r in enumerate(rules):
        s = r.get("suggestions", None)
        if not isinstance(s, list) or len(s) == 0:
            bad.append((i, r.get("rule_type"), r.get("family"), r.get("position"), r.get("condition")))
    if bad:
        print(f"\n[ERROR] {len(bad)} rules missing suggestions in {where}. Showing first {min(show, len(bad))}:")
        for i, rt, fam, pos, cond in bad[:show]:
            print(f"  idx={i} type={rt} family={fam} pos={pos} cond={cond}")
        raise AssertionError("Some rules missing suggestions after normalization")

def normalize_rule_suggestions(r: Dict) -> Dict:
    s = r.get("suggestions", None)
    if isinstance(s, list) and len(s) > 0:
        return r

    aa = r.get("suggested_aa")
    if aa:
        r["suggestions"] = [{
            "aa": aa,
            "rank": 1,
            ...
            "confidence_all": float(r.get("confidence_all", 0.0) or 0.0),
            "lift": float(r.get("lift", 0.0) or 0.0),
            "baseline_prob": float(r.get("baseline_prob", 0.0) or 0.0),
        }]
    else:
        r["suggestions"] = []
    return r
...
class CompensationAnalyzer:
    ...
    def __init__(self, positions: List[int]):
        ...
        # position -> Counter(aa) for baseline
        self.baseline: Dict[int, Counter] = defaultdict(Counter)
        # Statistics
    ...
    def extract_rules(self, min_support: int, min_confidence: float, 
                      min_lift: float, filter_definitional: bool = True) -> List[Dict]:
        ...
        # Precompute baseline probabilities
        baseline_prob: Dict[int, Dict[str, float]] = {}
        for pos, aa_counts in self.baseline.items():
            total = sum(aa_counts.values())
            if total > 0:
                baseline_prob[pos] = {aa: c / total for aa, c in aa_counts.items()}
        ...
        for (family, cond, pos), aa_counts in self.counts.items():
            ...
            base_probs = baseline_prob.get(pos, {})
            if not base_probs:
                continue
            
            # Get top AA for this condition
            ...
            rules.append({
                "condition": cond,
                "condition_tokens": [cond],  # Single condition, but structure supports multiple
                "position": f"IMGT{pos}",
                "position_num": pos,
                "suggested_aa": aa,
    ...
    def summarize(self) -> Dict:
        ...
        for family, pos_map in self.family_pos_aa_counts.items():
            ...
            for pos in sorted(pos_map.keys()):
                aa_counts = pos_map[pos]
                n_obs = sum(aa_counts.values())
                if n_obs == 0:
                    continue
                
                # Skip all-gap positions
                if len(aa_counts) == 1 and next(iter(aa_counts)) in ("-", "X", "*", ""):
                    continue
                
                # Major AA and conservation
                major_aa, major_count = aa_counts.most_common(1)[0]
                major_freq = major_count / n_obs
                
                # Shannon entropy (bits)
                entropy = 0.0
                for a, c in aa_counts.items():
                    if a in ("-", "X", "*", ""):
                        continue
                    p = c / n_obs
                    if p > 0:
...
class MIAnalyzer:
    """Compute pairwise MI between positions on a subsample."""
            ...
            for j in range(i + 1, P):
                ...
                for a in range(max_aa):
                    for b in range(max_aa):
                        ...
                        if pa <= 0 or pb <= 0:
                            continue
                        mi += pxy * math.log(pxy / (pa * pb + 1e-12) + 1e-12, 2)
...
def main():
    ap = argparse.ArgumentParser(description="VHH Analysis / Compensation Unified v7")
    ap.add_argument("--csv", required=True, help="IMGT-annotated VHH CSV")
    ...
    # Skip flags for visualization-only sections
    ap.add_argument("--skip-mi", action="store_true",
                    help="Skip MI computation (visualization only - heatmaps)")
    args = ap.parse_args()
    ...
    print(f"Total rows in CSV: {total_rows:,} (counted in {time.time()-t0:.1f}s)")
    print()
    ...
    mi_analyzer = MIAnalyzer(target_positions, max_seqs=args.mi_max_seqs) if not args.skip_mi else None
    ...
    if args.resume and os.path.exists(checkpoint_path):
        ...
        try:
            ...
            mi_analyzer = ckpt.get("mi_analyzer") if not args.skip_mi else None
    ...
    with open(args.csv, "r", newline="") as fh:
        ...
        for row in reader:
            ...
            hallmarks = row[hm_idx] if hm_idx and hm_idx < len(row) and row[hm_idx] else infer_hallmarks(imgt)
            family = row[fam_idx] if fam_idx and fam_idx < len(row) and row[fam_idx] else infer_family(imgt, aa_v_full)
    ...
    print(f"MI sequences: {mi_seqs:,}" + (" (skipped)" if args.skip_mi else ""))
    print()
    ...
    comp_rules = [normalize_rule_suggestions(r) for r in comp_rules]
    ...
    triplet_rules = [normalize_rule_suggestions(r) for r in triplet_rules]
    print(f"Triplet rules: {len(triplet_rules):,} ({time.time()-t0:.1f}s)")
    
    # Extract vernier archetypes
    t0 = time.time()
    archetypes = comp_analyzer.vernier_archetypes(min_support=max(args.min_support, 10000))
    print(f"Vernier archetypes: {len(archetypes):,} ({time.time()-t0:.1f}s)")
    
    # Extract MI pairs (if enabled)
    mi_pairs = []
    if mi_analyzer and not args.skip_mi:
        t0 = time.time()
    ...
    assert_rules_have_suggestions(all_rules, where="all_rules")
    ...
    if mi_pairs and not args.skip_mi:
        t0 = time.time()
