...
class CompensationAnalyzer:
    ...
    def extract_rules(self, min_condition_obs: int, min_aa_support: int,
        ...
        for (family, cond, pos), aa_counts in self.counts.items():
            # Exclude definitional/tautological rules if requested
            # Only block when BOTH: condition uses hallmarks/family AND position is a hallmark
            # Example blocked: hallmarks=YERL → IMGT42=Y (42 is part of YERL)
            # Example allowed: hallmarks=YERL → IMGT68=A (68 is not a hallmark)
...
def normalize_rule_suggestions(r: Dict) -> Dict:
    """
    Ensure every rule has a `suggestions` list.
    
    Compensation rules already have suggestions=[...] (grouped format).
    Triplet rules only have suggested_aa (legacy format).
    This normalizes both to have suggestions list.
    """
    # If already has non-empty suggestions list, return as-is
    if isinstance(r.get("suggestions"), list) and len(r["suggestions"]) > 0:
        return r
    
    # Build a 1-item suggestions list from legacy fields
    ...
    if aa:
        r["suggestions"] = [{
            ...
            "confidence_all": float(r.get("confidence_all", r.get("confidence", 0.0)) or 0.0),
            "lift": float(r.get("lift", 0.0) or 0.0),
            "baseline_prob": float(r.get("baseline_prob", 0.0) or 0.0),
        }]
    else:
        # Truly no suggestion - keep empty list for consistency
        r["suggestions"] = []
    
    return r


def assert_rules_have_suggestions(rules: List[Dict], where: str = "rules") -> None:
    """
    Assert all rules have non-empty suggestions list.
    Prints diagnostic info on failure.
    """
    bad_rules = [r for r in rules if not r.get("suggestions")]
    if bad_rules:
        print(f"ERROR: {len(bad_rules)} rules missing suggestions in {where}")
        for r in bad_rules[:5]:  # Show first 5 offenders
            print(f"  - position={r.get('position')} family={r.get('family')} type={r.get('rule_type')}")
        raise AssertionError(f"{len(bad_rules)} rules missing suggestions in {where}")
...
def main():
    ap = argparse.ArgumentParser(
        description="VHH Analysis / Compensation Unified v7.7",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Standard run (sensible defaults - infers family, all FR positions, legacy pickles)
  python vhh_analysis_unified_v7.py --csv data/*.csv -o models/analysis
...
"""
    ...
    ap.add_argument("--positions", choices=["all", "vernier", "fr3fr4"], default="all",
                    help="FR positions for rules: all (~85), vernier (16), fr3fr4 (~50)")
    ...
    ap.add_argument("--infer-family", action="store_true", default=True,
                    help="Infer family from sequence (default: ON)")
    ap.add_argument("--no-infer-family", action="store_false", dest="infer_family",
                    help="Use CSV family column instead of inferring")
    ...
    # Normalize to guarantee every rule has suggestions=[...]
    all_rules = [normalize_rule_suggestions(r) for r in all_rules]
    
    # Assert all rules have suggestions (diagnostic on failure)
