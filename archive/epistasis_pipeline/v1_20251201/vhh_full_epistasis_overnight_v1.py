...
def classify_family(positions: Dict[str, str], cdr3: str) -> str:
    pos42 = positions.get('FR2_4', '-')
    pos50 = positions.get('FR2_12', '-')
    pos52 = positions.get('FR2_14', '-')
...
class ConditionalModeler:
    ...
    def train_models(self, min_samples: int = 2000, min_classes: int = 2,
        ...
        print(f"    Trained {model_count} models total")
        return results
...
class MutualInformationAnalyzer:
    ...
    def compute_matrix(self) -> Tuple[np.ndarray, List[str]]:
        print(f"    Computing MI matrix ({self.n_positions}x{self.n_positions}) from {self.n_added:,} sequences...")
...
def process_npz(npz_path: str, analyzers: Dict) -> int:
    ...
    for i in range(len(sequences)):
        try:
            ...
            family = classify_family(positions, cdr3)
...
def main():
    ...
    print("="*70)
    print("VHH FULL EPISTASIS ANALYSIS - OVERNIGHT v2")
    ...
    for i, npz_file in enumerate(npz_files):
        ...
        print(f"  [{i+1}/{len(npz_files)}] {npz_file}...", end=' ', flush=True)
        ...
        print(f"{count:,} seqs (total: {total:,}, {rate:.0f}/sec)")
