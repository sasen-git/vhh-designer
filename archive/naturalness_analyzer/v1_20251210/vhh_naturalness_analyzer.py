...
def extract_cdr3_from_sequence(sequence: str) -> str:
    ...
    # Look for C...WG pattern
    match = re.search(r'C([A-Z]{3,35})W[GS]', seq)
    if match:
        return 'C' + match.group(1)
    
    # Fallback: look for C around position 95-105
    c_pos = seq.rfind('C', 80, 110)
    if c_pos > 0:
...
def extract_frameworks_from_sequence(sequence: str) -> Optional[FrameworkProfile]:
    ...
    # Find CDR3
    cdr3_match = re.search(r'C([A-Z]{3,35})W[GS]', seq[fr2_end:])
    if not cdr3_match:
        return None
    
    cdr3_start = fr2_end + cdr3_match.start()
    cdr3_end = fr2_end + cdr3_match.end() - 2
    
    profile = FrameworkProfile(
        name='Extracted',
        fr1=seq[:fr2_start],
        fr2=seq[fr2_start:fr2_end],
        fr3=seq[fr2_end + 17:cdr3_start],  # Skip CDR2 (~17 aa)
        fr4=seq[cdr3_end:]
    )
