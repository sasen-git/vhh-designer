...
import json
import platform
import math  # Added for histogram functionality
...
# Excel Export
# ------------------------
def export_to_excel(csv_path: str, results_csv_path: str, output_folder: str, query_cdrs: Dict = None) -> Optional[str]:
    """
    ...
    try:
        ...
        with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
            ...
            if query_cdrs:
                query_info_data = []
                query_info_data.append(['QUERY INFORMATION', ''])
                query_info_data.append(['', ''])
                query_info_data.append(['Full Query Sequence:', query_cdrs.get('full', 'N/A')])
                query_info_data.append(['Query Length:', f"{len(query_cdrs.get('full', ''))} aa" if query_cdrs.get('full') else 'N/A'])
                query_info_data.append(['', ''])
                query_info_data.append(['EXTRACTED CDRs:', ''])
                query_info_data.append(['', ''])
                query_info_data.append(['CDR1 Sequence:', query_cdrs.get('cdr1', 'N/A')])
                query_info_data.append(['CDR1 Length:', f"{len(query_cdrs.get('cdr1', ''))} aa" if query_cdrs.get('cdr1') else 'N/A'])
                query_info_data.append(['', ''])
                query_info_data.append(['CDR2 Sequence:', query_cdrs.get('cdr2', 'N/A')])
                query_info_data.append(['CDR2 Length:', f"{len(query_cdrs.get('cdr2', ''))} aa" if query_cdrs.get('cdr2') else 'N/A'])
                query_info_data.append(['', ''])
                query_info_data.append(['CDR3 Sequence:', query_cdrs.get('cdr3', 'N/A')])
                query_info_data.append(['CDR3 Length:', f"{len(query_cdrs.get('cdr3', ''))} aa" if query_cdrs.get('cdr3') else 'N/A'])
...
def print_ascii_histogram(values: List[float], bin_width: float = 1.0, label: str = "", max_bar_width: int = 50):
    ...
    Args:
        ...
        bin_width: Width of each bin (default 1.0 for amino acid counts)
    ...
    for i, count in enumerate(counts):
        ...
        if bin_width == 1.0:  # Integer bins (for aa counts)
            bin_label = f"{int(bin_start):3d}"
            if bin_start != bin_end - 1:
                bin_label = f"{int(bin_start):3d}-{int(bin_end-1):3d}"
        else:
    ...
    print(f"  Std Dev: {np.std(clean_values):.1f}")
...
def extract_cdr3_from_npz(int_row):
    ...
    FIXED: Properly handles truncated sequences without FR4.
    """
    ...
    # CRITICAL FIX: Look for valid FR4 motifs, not just any W
    # Common FR4 starting motifs in antibodies (especially camelids)
    fr4_motifs = [
        'WGQG', 'WGKG', 'WGRG', 'WGAG', 'WGLG', 'WGEG',
        'WGNG', 'WGSG', 'WGTG', 'WGVG', 'WGMG', 'WGPG',
        'WGDG', 'WGHG', 'WGYG', 'WGFG', 'WGCG', 'WGIG',
        'WGWG', 'WG'  # At minimum, should have WG
    ]
    
    # Find the earliest valid FR4 motif
    fr4_start = -1
    for motif in fr4_motifs:
        pos = cdr3_aa.find(motif)
        if pos > 0:  # Must be after position 0 (CDR3 must have at least 1 residue)
            if fr4_start == -1 or pos < fr4_start:
                fr4_start = pos
    
    # If we found a valid FR4 motif
    if fr4_start > 0:
        cdr3 = cdr3_aa[:fr4_start]
        
        # Additional validation:
        # 1. CDR3 should be at least 3 amino acids (very rare to be shorter)
        # 2. CDR3 should not be longer than 25 amino acids (extremely rare to be longer)
        if 3 <= len(cdr3) <= 25:
            return cdr3
        else:
            # Invalid CDR3 length - likely misidentified
            return ""
    else:
        # No valid FR4 motif found - this is a truncated sequence
        # DO NOT return a fake CDR3 from the tail end
        return ""
...
def get_user_input_interactive():
    ...
    DB_BASE_PATHS = [
        ...
        os.path.expanduser("~/KA-Search/extracted/oas-small/oasdb_20230111/Heavy"),
        os.path.expanduser("~/KA-Search/extracted/oas-tiny/oasdb_20230111/Heavy"),
    ...
    query_loaded_from_saved = False  # Track if loaded from saved queries
    saved_files = []
    ...
    while True:
        ...
        # Offer to save (only if not already saved)
        if 'query_loaded_from_saved' in locals() and query_loaded_from_saved:
            # Already saved, don't ask or duplicate
            pass
        else:
...
def sample_and_analyze_shard(npz_file: str, query_cdrs: Dict, sample_size: int, 
    ...
    try:
        ...
        for idx in indices:
            ...
            # ALWAYS store lengths for histogram functionality
            full_seq = extract_whole_from_npz(row)
            result['len_full'] = len(full_seq) if full_seq else 0
            
            cdr3_seq = extract_cdr3_from_npz(row)
            result['len_cdr3'] = len(cdr3_seq) if cdr3_seq else 0
            
            # Store identities if any were calculated
            if identities:
                result["avg_id"] = np.mean(identities)
            else:
                # No matches but we still want to track this sequence for statistics
                result["avg_id"] = 0.0
            
            # ALWAYS append result to track all sampled sequences
...
def run_interactive_analysis(db_root: str, query_seq: str, regions: List[str],
    ...
    # ADDED: Ask about histogram visualization
    if len(combined) > 0:
        print("\n" + "="*80)
        print("LENGTH DISTRIBUTION ANALYSIS")
        print("="*80)
        
        show_hist = input("\nShow length histograms from analysis sample? (y/n): ").strip().lower()
        if show_hist in ['y', 'yes']:
            # Show full sequence length histogram if available
            if 'len_full' in combined.columns:
                full_lengths = combined['len_full'].dropna().tolist()
                if full_lengths:
                    print_ascii_histogram(full_lengths, bin_width=5, 
                                        label="Full Sequence Length Distribution (5 aa bins)")
            
            # Show CDR3 length histogram if available
            if 'len_cdr3' in combined.columns:
                cdr3_lengths = combined['len_cdr3'].dropna().tolist()
                # Filter out zeros (no CDR3)
                cdr3_lengths = [l for l in cdr3_lengths if l > 0]
                if cdr3_lengths:
                    print_ascii_histogram(cdr3_lengths, bin_width=1,
                                        label="CDR3 Length Distribution (1 aa bins)")
