...
def extract_regions_from_sequence(aa_v_full: str, cdr1: str, cdr2: str, cdr3: str) -> dict:
    ...
    try:
        ...
        # FR2 (IMGT 41-54) - key hallmark region
        # Canonical VH/VHH FR2 motif anchors W at IMGT41
...
def main():
    ...
    parser.add_argument('--npz', '-n', help='NPZ file or glob pattern')
    parser.add_argument('--csv', help='CSV file with aa_v_full, cdr1, cdr2, cdr3, fr1, fr2, fr3, fr4 columns')
    ...
    if not args.npz and not args.csv:
        parser.error("Either --npz or --csv is required")
    
    input_source = args.csv if args.csv else args.npz
    ...
    # CSV processing (streaming with chunks)
    if args.csv:
        import pandas as pd
        ...
        for chunk in tqdm(pd.read_csv(args.csv, chunksize=chunk_size, low_memory=False), desc="Processing chunks"):
            ...
            for idx, row in chunk.iterrows():
                ...
                aa_v_full = str(row.get('aa_v_full', '') or '')
                cdr1 = str(row.get('cdr1', '') or '')
                cdr2 = str(row.get('cdr2', '') or '')
                cdr3 = str(row.get('cdr3', '') or '')
                fr1 = str(row.get('fr1', '') or '')
                fr2 = str(row.get('fr2', '') or '')
                fr3 = str(row.get('fr3', '') or '')
                fr4 = str(row.get('fr4', '') or '')
                
                # Build IMGT positions from FRs and CDRs
                positions = {}
                ...
                # FR2 (IMGT 39-55) - full span as extracted
                for i, aa in enumerate(fr2):
                    positions[39 + i] = aa
                ...
                if not positions or not cdr3:
                    continue
                
                processor.process_sequence(positions, cdr1, cdr2, cdr3, aa_v_full)
                total_processed += 1
            
            gc.collect()
        
        print(f"\nProcessed {total_processed:,} sequences from CSV")
